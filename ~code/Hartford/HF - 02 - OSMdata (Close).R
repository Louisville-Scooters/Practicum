...
boundary = HF_Census_geoinfo # service area boundary              ####
census_ct = HF_Census_ct # census tract ecosocia data ####
#origin_ct = HF_open_ct #census tract with count of trip origins
census_geoinfo = HF_Census_geoinfo                    ####
#*****************************************************####
...
# street
HF_street <- st_read('https://opendata.arcgis.com/datasets/06146b5fb22d445daf9ff2273d72540d_11.geojson') %>%
  st_transform(2272) 
ggplot() +
  geom_sf(data = HF_Census_geoinfo) +
  geom_sf(data = st_intersection(HF_street,HF_Census_geoinfo))


HF_street_ct_len <- st_intersection(HF_street, HF_Census_geoinfo) %>%
  mutate(length = as.numeric(st_length(.))*0.000189394) %>%
  group_by(GEOID) %>%
  summarise(street_length = sum(length)) %>%
  st_set_geometry(NULL) %>%
  merge(HF_Census_geoinfo, on='GEOID', all.y=T) %>%
  st_as_sf()

HF_street_ct_len$street_length <- replace_na(HF_street_ct_len$street_length,0)
...
HF_spatial_census <- spatial_census
HF_spatial_census <- merge(HF_spatial_census, as.data.frame(HF_street_ct_len) %>% dplyr::select(GEOID, street_length), by='GEOID')
HF_spatial_census <- HF_spatial_census %>%
  subset(HF_spatial_census$street_length>0)
# ratio
HF_spatial_census$ratio_retail <- HF_spatial_census$count_retail/length(HF_spatial_census$count_retail)[1]
HF_spatial_census$ratio_office <- HF_spatial_census$count_office/length(HF_spatial_census$count_office)[1]
HF_spatial_census$ratio_restaurant <- HF_spatial_census$count_restaurant/length(HF_spatial_census$count_office)[1]
HF_spatial_census$ratio_public_transport <- HF_spatial_census$count_pubtran/length(HF_spatial_census$count_pubtran)[1]
HF_spatial_census$ratio_leisure <- HF_spatial_census$count_leisure/length(HF_spatial_census$count_leisure)[1]
HF_spatial_census$ratio_tourism <- HF_spatial_census$count_tourism/length(HF_spatial_census$count_tourism)[1]
HF_spatial_census$ratio_college <- HF_spatial_census$count_college/length(HF_spatial_census$count_college)[1]
HF_spatial_census$ratio_cycleway <- HF_spatial_census$total_length/sum(HF_spatial_census$total_length)
HF_spatial_census$ratio_street <- HF_spatial_census$street_length %>% replace_na(0)/sum(HF_spatial_census$street_length %>% replace_na(0))


HF_spatial_census <- merge(HF_spatial_census, as.data.frame(HF_Census) %>% dplyr::select(-geometry), by='GEOID')
HF_Census_q5 <- apply(HF_Census %>% st_set_geometry(NULL) %>% dplyr::select(-GEOID), 2, q5)
HF_Census_q5 <- cbind(values = as.data.frame(HF_Census), q5 = HF_Census_q5)

# HF_spatial_census_RDS <- file.path(data_directory, "~RData/Hartford/HF_spatial_census")
# saveRDS(HF_spatial_census,
#         file = HF_spatial_census_RDS)
HF_spatial_census <- readRDS(HF_spatial_census_RDS)

# HF_Census_q5_RDS <- file.path(data_directory, "~RData/Hartford/HF_Census_q5")
# saveRDS(HF_Census_q5,
#         file = HF_Census_q5_RDS)
HF_Census_q5 <- readRDS(HF_Census_q5_RDS)