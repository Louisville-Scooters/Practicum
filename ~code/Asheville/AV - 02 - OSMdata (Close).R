...
boundary = AV_Census_geoinfo # service area boundary              ####
census_ct = AV_Census_ct # census tract ecosocia data ####
#origin_ct = AV_open_ct #census tract with count of trip origins
census_geoinfo = AV_Census_geoinfo                    ####
#*****************************************************####
...
# street
AV_street_path <- file.path(data_directory,
                       "Asheville/Basemap_routes.shp")
AV_street <- st_read('https://opendata.arcgis.com/datasets/a85edc734ebf4bb098eaea4198cfe04d_3.geojson') %>%
  st_transform(2272)

# ggplot() +
#   geom_sf(data = AV_Census_geoinfo) +
#   geom_sf(data = st_intersection(AV_street,AV_Census_geoinfo))


AV_street_ct_len <- st_intersection(AV_street,AV_Census_geoinfo) %>%
  mutate(length = as.numeric(st_length(.))*0.000189394) %>%
  group_by(GEOID) %>%
  summarise(street_length = sum(length)) %>%
  st_set_geometry(NULL) %>%
  merge(AV_Census_geoinfo, on='GEOID', all.y=T) %>%
  st_as_sf()

AV_street_ct_len$street_length <- replace_na(AV_street_ct_len$street_length,0)
...
AV_spatial_census <- spatial_census
AV_spatial_census <- merge(AV_spatial_census, as.data.frame(AV_street_ct_len) %>% dplyr::select(GEOID, street_length), by='GEOID')
# ratio
AV_spatial_census$ratio_retail <- AV_spatial_census$count_retail/length(AV_spatial_census$count_retail)[1]
AV_spatial_census$ratio_office <- AV_spatial_census$count_office/length(AV_spatial_census$count_office)[1]
AV_spatial_census$ratio_restaurant <- AV_spatial_census$count_restaurant/length(AV_spatial_census$count_office)[1]
AV_spatial_census$ratio_public_transport <- AV_spatial_census$count_pubtran/length(AV_spatial_census$count_pubtran)[1]
AV_spatial_census$ratio_leisure <- AV_spatial_census$count_leisure/length(AV_spatial_census$count_leisure)[1]
AV_spatial_census$ratio_tourism <- AV_spatial_census$count_tourism/length(AV_spatial_census$count_tourism)[1]
AV_spatial_census$ratio_college <- AV_spatial_census$count_college/length(AV_spatial_census$count_college)[1]
AV_spatial_census$ratio_cycleway <- AV_spatial_census$total_length/sum(AV_spatial_census$total_length)
AV_spatial_census$ratio_street <- AV_spatial_census$street_length %>% replace_na(0)/sum(AV_spatial_census$street_length %>% replace_na(0))


AV_spatial_census <- merge(AV_spatial_census, as.data.frame(AV_Census) %>% dplyr::select(-geometry), by='GEOID')
AV_Census_q5 <- apply(AV_Census %>% st_set_geometry(NULL) %>% dplyr::select(-GEOID), 2, q5)
AV_Census_q5 <- cbind(values = as.data.frame(AV_Census), q5 = AV_Census_q5)

# AV_spatial_census_RDS <- file.path(data_directory, "~RData/Asheville/AV_spatial_census")
# saveRDS(AV_spatial_census,
#         file = AV_spatial_census_RDS)
AV_spatial_census <- readRDS(AV_spatial_census_RDS)

# AV_Census_q5_RDS <- file.path(data_directory, "~RData/Asheville/AV_Census_q5")
# saveRDS(AV_Census_q5,
#         file = AV_Census_q5_RDS)
AV_Census_q5 <- readRDS(AV_Census_q5_RDS)