...
predict_HS <- ggplot()+
  geom_sf(data = HS_trimmed_result %>% na.omit(), aes(fill=q5(Predicted.CNT))) +
  scale_fill_viridis_d(labels=qBr(HS_trimmed_result,"Predicted.CNT"),name="Quintile\nBreaks")+
...
Dist_HS <- ggplot()+
  geom_histogram(data = as.data.frame(HS_trimmed_result) %>% na.omit(), aes(round(Predicted.CNT)), fill='#453781FF', bins=30) +
  labs(title='Distribution of Trip Count in Houston', x='Predicted trip count') +
  plotTheme
