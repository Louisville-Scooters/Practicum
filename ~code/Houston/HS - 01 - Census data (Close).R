...
HS_Census_raw <- get_acs(geography = "tract", 
...
HS_Census_geoinfo <- HS_Census_raw %>%
  dplyr::select(GEOID, geometry) %>%
  na.omit()

# extract centroid of each census tract
HS_Census_geoinfo <- HS_Census_geoinfo %>% 
  mutate(centroid_X = st_coordinates(st_centroid(HS_Census_geoinfo))[, 1],
         centroid_Y = st_coordinates(st_centroid(HS_Census_geoinfo))[, 2])

HS_Census <- HS_Census_raw %>% 
...
HS_tract_list <- HS_Census_geoinfo$GEOID

HS_Census_ct <- HS_Census %>%
  filter(HS_Census$GEOID %in% HS_tract_list) %>%
  st_set_geometry(NULL)

# rejoin geometry infor from ct_HS
HS_Census_ct <- merge(HS_Census_geoinfo, HS_Census_ct, by = 'GEOID')
