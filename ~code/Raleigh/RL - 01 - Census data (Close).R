...
RL_Census_raw <- get_acs(geography = "tract", 
...
RL_Census_geoinfo <- RL_Census_raw %>%
  dplyr::select(GEOID, geometry) %>%
  na.omit()

# extract centroid of each census tract
RL_Census_geoinfo <- RL_Census_geoinfo %>% 
  mutate(centroid_X = st_coordinates(st_centroid(RL_Census_geoinfo))[, 1],
         centroid_Y = st_coordinates(st_centroid(RL_Census_geoinfo))[, 2])

RL_Census <- RL_Census_raw %>% 
...
RL_tract_list <- RL_Census_geoinfo$GEOID

RL_Census_ct <- RL_Census %>%
  filter(RL_Census$GEOID %in% RL_tract_list) %>%
  st_set_geometry(NULL)

# rejoin geometry infor from ct_RL
RL_Census_ct <- merge(RL_Census_geoinfo, RL_Census_ct, by = 'GEOID')
