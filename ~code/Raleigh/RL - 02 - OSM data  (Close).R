...
boundary = RL_Census_geoinfo # service area boundary              ####
census_ct = RL_Census_ct # census tract ecosocia data ####
#origin_ct = RL_open_ct #census tract with count of trip origins
census_geoinfo = RL_Census_geoinfo                    ####
#*****************************************************####
...
# street
RL_street <- st_read('http://data-phl.opendata.arcgis.com/datasets/c36d828494cd44b5bd8b038be696c839_0.geojson') %>%
  st_transform(2272)

# RL_street <- RL_street %>% st_join(RL_Census_geoinfo)

RL_street_ct_len <- st_intersection(RL_street,RL_Census_geoinfo) %>%
  mutate(length = as.numeric(st_length(.))*0.000189394) %>%
  group_by(GEOID) %>%
  summarise(street_length = sum(length)) %>%
  st_set_geometry(NULL) %>%
  merge(RL_Census_geoinfo, on='GEOID', all.y=T) %>%
  st_as_sf()

RL_street_ct_len$street_length <- replace_na(RL_street_ct_len$street_length,0)
...
RL_spatial_census <- spatial_census
RL_spatial_census <- merge(RL_spatial_census, as.data.frame(RL_street_ct_len) %>% dplyr::select(GEOID, street_length), by='GEOID')
# ratio
RL_spatial_census$ratio_retail <- RL_spatial_census$count_retail/length(RL_spatial_census$count_retail)[1]
RL_spatial_census$ratio_office <- RL_spatial_census$count_office/length(RL_spatial_census$count_office)[1]
RL_spatial_census$ratio_restaurant <- RL_spatial_census$count_restaurant/length(RL_spatial_census$count_office)[1]
RL_spatial_census$ratio_public_transport <- RL_spatial_census$count_pubtran/length(RL_spatial_census$count_pubtran)[1]
RL_spatial_census$ratio_leisure <- RL_spatial_census$count_leisure/length(RL_spatial_census$count_leisure)[1]
RL_spatial_census$ratio_tourism <- RL_spatial_census$count_tourism/length(RL_spatial_census$count_tourism)[1]
RL_spatial_census$ratio_college <- RL_spatial_census$count_college/length(RL_spatial_census$count_college)[1]
RL_spatial_census$ratio_cycleway <- RL_spatial_census$total_length/sum(RL_spatial_census$total_length)
RL_spatial_census$ratio_street <- RL_spatial_census$street_length/sum(RL_spatial_census$street_length)


RL_spatial_census <- merge(RL_spatial_census, as.data.frame(RL_Census) %>% dplyr::select(-geometry), by='GEOID')
RL_Census_q5 <- apply(RL_Census %>% st_set_geometry(NULL) %>% dplyr::select(-GEOID), 2, q5)
RL_Census_q5 <- cbind(values = as.data.frame(RL_Census), q5 = RL_Census_q5)

# RL_spatial_census_RDS <- file.path(data_directory, "~RData/Raleigh/RL_spatial_census")
# saveRDS(RL_spatial_census,
#         file = RL_spatial_census_RDS)
RL_spatial_census <- readRDS(RL_spatial_census_RDS)

# RL_Census_q5_RDS <- file.path(data_directory, "~RData/Raleigh/RL_Census_q5")
# saveRDS(RL_Census_q5,
#         file = RL_Census_q5_RDS)
RL_Census_q5 <- readRDS(RL_Census_q5_RDS)