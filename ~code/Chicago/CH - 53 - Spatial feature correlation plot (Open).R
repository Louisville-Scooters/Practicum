...
glimpse(CH_spatial_census)

CH_ori_spatial_correlation.long <-
...
CH_ori_spatial_correlation.cor <-
  CH_ori_spatial_correlation.long %>%
  group_by(Variable) %>%
  summarize(correlation = cor(Value, origins_cnt, use = "complete.obs"))

ggplot(CH_ori_spatial_correlation.long, aes(Value, origins_cnt)) +
  geom_point(size = 0.1) +
  geom_text(data = CH_ori_spatial_correlation.cor, aes(label = paste("r =", round(correlation, 2))),
