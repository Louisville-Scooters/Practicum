...
boundary = JC_Census_geoinfo # service area boundary              ####
census_ct = JC_Census_ct # census tract ecosocia data ####
#origin_ct = JC_open_ct #census tract with count of trip origins
census_geoinfo = JC_Census_geoinfo                    ####
#*****************************************************####
...
# street
RL_street_path <- file.path(data_directory,
                       "Jercey City/1985f3cd-63cc-48ec-9888-896515ed8c19202044-1-kvonml.9famd.shp")
RL_street <- st_read('https://opendata.arcgis.com/datasets/3d28e04feaa34079bfb8fc1efc88a0e0_1.geojson') %>%
  st_transform(2272)


JC_street_ct_len <- st_intersection(RL_street,JC_Census_geoinfo) %>%
  mutate(length = as.numeric(st_length(.))*0.000189394) %>%
  group_by(GEOID) %>%
  summarise(street_length = sum(length)) %>%
  st_set_geometry(NULL) %>%
  merge(JC_Census_geoinfo, on='GEOID', all.y=T) %>%
  st_as_sf()

JC_street_ct_len$street_length <- replace_na(JC_street_ct_len$street_length,0)
...
JC_spatial_census <- spatial_census
JC_spatial_census <- merge(JC_spatial_census, as.data.frame(JC_street_ct_len) %>% dplyr::select(GEOID, street_length), by='GEOID')
# ratio
JC_spatial_census$ratio_retail <- JC_spatial_census$count_retail/length(JC_spatial_census$count_retail)[1]
JC_spatial_census$ratio_office <- JC_spatial_census$count_office/length(JC_spatial_census$count_office)[1]
JC_spatial_census$ratio_restaurant <- JC_spatial_census$count_restaurant/length(JC_spatial_census$count_office)[1]
JC_spatial_census$ratio_public_transport <- JC_spatial_census$count_pubtran/length(JC_spatial_census$count_pubtran)[1]
JC_spatial_census$ratio_leisure <- JC_spatial_census$count_leisure/length(JC_spatial_census$count_leisure)[1]
JC_spatial_census$ratio_tourism <- JC_spatial_census$count_tourism/length(JC_spatial_census$count_tourism)[1]
JC_spatial_census$ratio_college <- JC_spatial_census$count_college/length(JC_spatial_census$count_college)[1]
JC_spatial_census$ratio_cycleway <- JC_spatial_census$total_length/sum(JC_spatial_census$total_length)
JC_spatial_census$ratio_street <- JC_spatial_census$street_length/sum(JC_spatial_census$street_length)


JC_spatial_census <- merge(JC_spatial_census, as.data.frame(JC_Census) %>% dplyr::select(-geometry), by='GEOID')
JC_Census_q5 <- apply(JC_Census %>% st_set_geometry(NULL) %>% dplyr::select(-GEOID), 2, q5)
JC_Census_q5 <- cbind(values = as.data.frame(JC_Census), q5 = JC_Census_q5)

# JC_spatial_census_RDS <- file.path(data_directory, "~RData/Jersey City/JC_spatial_census")
# saveRDS(JC_spatial_census,
#         file = JC_spatial_census_RDS)
JC_spatial_census <- readRDS(JC_spatial_census_RDS)

# JC_Census_q5_RDS <- file.path(data_directory, "~RData/Jersey City/JC_Census_q5")
# saveRDS(JC_Census_q5,
#         file = JC_Census_q5_RDS)
JC_Census_q5 <- readRDS(JC_Census_q5_RDS)