...
JC_Census_raw <- get_acs(geography = "tract", 
...
JC_Census_geoinfo <- JC_Census_raw %>%
  dplyr::select(GEOID, geometry) %>%
  na.omit()

# extract centroid of each census tract
JC_Census_geoinfo <- JC_Census_geoinfo %>% 
  mutate(centroid_X = st_coordinates(st_centroid(JC_Census_geoinfo))[, 1],
         centroid_Y = st_coordinates(st_centroid(JC_Census_geoinfo))[, 2])

JC_Census <- JC_Census_raw %>% 
...
JC_tract_list <- JC_Census_geoinfo$GEOID

JC_Census_ct <- JC_Census %>%
  filter(JC_Census$GEOID %in% JC_tract_list) %>%
  st_set_geometry(NULL)

# rejoin geometry infor from ct_JC
JC_Census_ct <- merge(JC_Census_geoinfo, JC_Census_ct, by = 'GEOID')
