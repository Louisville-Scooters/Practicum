...
AU_KC_LV_Census <- rbind(AU_Census %>% st_set_geometry(NULL), 
                         KC_Census %>% st_set_geometry(NULL), 
                         LV_Census %>% st_set_geometry(NULL))

AU_KC_LV_Census_gather <- AU_KC_LV_Census %>%
  dplyr::select(-GEOID) %>%
  gather(key='Variable',"Value", -city)

ggplot() +
  geom_boxplot(data = as.data.frame(AU_KC_LV_Census_gather), aes(x=city, y=Value, fill=city)) +
