census_ratio <- colnames(Model_clean)[8:16]
census_var <- colnames(Model_clean)[c(2:7, 15,16)]
osm_var <- colnames(Model_clean)[c(15:27)]

Model_clean_census <- Model_clean %>% dplyr::select(census_var)#, census_ratio)
Model_clean_osm <- Model_clean %>% dplyr::select(osm_var)
Model_clean_ratio <- Model_clean %>% dplyr::select(census_ratio)
...
train.set.census <- training(data_split) %>% dplyr::select(census_var)#, census_ratio)
test.set.census  <- testing(data_split) %>% dplyr::select(census_var)#, census_ratio)
...
#### FOR CENSUS RATIO ####
# split the data into training and test
train.set.cratio <- training(data_split) %>% dplyr::select(census_ratio)
test.set.cratio  <- testing(data_split) %>% dplyr::select(census_ratio)


### Cross Validation
## LOGOCV on City with group_vfold_cv()
cv_splits_geo <- group_vfold_cv(train.set.cratio,  strata = "ORIGINS_CNT", group = "CITY")

### Create Recipes ###
# Feature Creation
model_rec <- recipe(ORIGINS_CNT ~ ., data = train.set.cratio) %>%
  update_role(CITY, new_role = "CITY") %>%
  step_other(CITY, threshold = 0.005) %>% #pool infrequently occurrin values into an "other" category.
  step_dummy(all_nominal(), -CITY) %>%
  #  step_log(ORIGINS_CNT) %>%  #has zero, cannot log 
...
rf_val_fit_geo_cratio <- rf_best_wf %>% 
  last_fit(split     = data_split,
           control   = control,
           metrics   = metric_set(rmse, rsq))

rf_best_OOF_preds_cratio <- collect_predictions(rf_tuned) %>% 
  filter(mtry  == rf_best_params$mtry[1] & min_n == rf_best_params$min_n[1])

rf_val_pred_geo_cratio <- collect_predictions(rf_val_fit_geo_cratio)

# compare the pred and actual data
OOF_preds_cratio <- data.frame(dplyr::select(rf_best_OOF_preds_cratio, .pred, ORIGINS_CNT)) %>%
  mutate(#.pred = exp(.pred),
    # ORIGINS_CNT = exp(ORIGINS_CNT),
    RMSE = yardstick::rmse_vec(ORIGINS_CNT, .pred),
    MAE  = yardstick::mae_vec(ORIGINS_CNT, .pred))
...
# OOF predicted versus actual
ggplot(OOF_preds_cratio, aes(x =.pred, y = ORIGINS_CNT)) +
  geom_point(alpha = 0.3) +
  geom_abline(linetype = "dashed", color = "red") +
  geom_smooth(method = "lm", color = "orange") +
  coord_equal() +
...
val_preds_cratio <- data.frame(rf_val_pred_geo_cratio, model = "rf") %>% 
  left_join(., Model_clean_ratio %>% 
              rowid_to_column(var = ".row") %>% 
              dplyr::select(CITY, .row), 
            by = ".row") %>%
  mutate(# .pred = exp(.pred),
...
# Validation Predicted vs. actual
ggplot(val_preds_cratio, aes(x =.pred, y = ORIGINS_CNT)) +
  geom_point() +
  geom_abline(linetype = "dashed", color = "red") +
  geom_smooth(method = "lm", color = "orange") +
  coord_equal() +
...
final_data$.pred_cratio <- data.frame(rf_val_pred_geo_cratio)$.pred

data_split_2nd_model <- initial_split(final_data, strata = "ORIGINS_CNT", prop = 0.75)
train.set <- training(data_split_2nd_model)
test.set  <- testing(data_split_2nd_model)
