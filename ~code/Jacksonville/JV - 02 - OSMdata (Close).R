...
boundary = JV_Census_geoinfo # service area boundary              ####
census_ct = JV_Census_ct # census tract ecosocia data ####
#origin_ct = JV_open_ct #census tract with count of trip origins
census_geoinfo = JV_Census_geoinfo                    ####
#*****************************************************####
...
# street
JV_street_path <- file.path(data_directory,
                       "Jacksonville/Basemap_routes.shp")
JV_street <- st_read(JV_street_path) %>%
  st_transform(2272) %>%
  st_zm()

# ggplot() +
#   geom_sf(data = JV_Census_geoinfo) +
#   geom_sf(data = st_intersection(JV_street,JV_Census_geoinfo))


JV_street_ct_len <- st_intersection(JV_street,JV_Census_geoinfo) %>%
  mutate(length = as.numeric(st_length(.))*0.000189394) %>%
  group_by(GEOID) %>%
  summarise(street_length = sum(length)) %>%
  st_set_geometry(NULL) %>%
  merge(JV_Census_geoinfo, on='GEOID', all.y=T) %>%
  st_as_sf()

JV_street_ct_len$street_length <- replace_na(JV_street_ct_len$street_length,0)
...
JV_spatial_census <- spatial_census
JV_spatial_census <- merge(JV_spatial_census, as.data.frame(JV_street_ct_len) %>% dplyr::select(GEOID, street_length), by='GEOID')
# ratio
JV_spatial_census$ratio_retail <- JV_spatial_census$count_retail/length(JV_spatial_census$count_retail)[1]
JV_spatial_census$ratio_office <- JV_spatial_census$count_office/length(JV_spatial_census$count_office)[1]
JV_spatial_census$ratio_restaurant <- JV_spatial_census$count_restaurant/length(JV_spatial_census$count_office)[1]
JV_spatial_census$ratio_public_transport <- JV_spatial_census$count_pubtran/length(JV_spatial_census$count_pubtran)[1]
JV_spatial_census$ratio_leisure <- JV_spatial_census$count_leisure/length(JV_spatial_census$count_leisure)[1]
JV_spatial_census$ratio_tourism <- JV_spatial_census$count_tourism/length(JV_spatial_census$count_tourism)[1]
JV_spatial_census$ratio_college <- JV_spatial_census$count_college/length(JV_spatial_census$count_college)[1]
JV_spatial_census$ratio_cycleway <- JV_spatial_census$total_length/sum(JV_spatial_census$total_length)
JV_spatial_census$ratio_street <- JV_spatial_census$street_length %>% replace_na(0)/sum(JV_spatial_census$street_length %>% replace_na(0))


JV_spatial_census <- merge(JV_spatial_census, as.data.frame(JV_Census) %>% dplyr::select(-geometry), by='GEOID')
JV_Census_q5 <- apply(JV_Census %>% st_set_geometry(NULL) %>% dplyr::select(-GEOID), 2, q5)
JV_Census_q5 <- cbind(values = as.data.frame(JV_Census), q5 = JV_Census_q5)

# JV_spatial_census_RDS <- file.path(data_directory, "~RData/Jacksonville/JV_spatial_census")
# saveRDS(JV_spatial_census,
#         file = JV_spatial_census_RDS)
JV_spatial_census <- readRDS(JV_spatial_census_RDS)

# JV_Census_q5_RDS <- file.path(data_directory, "~RData/Jacksonville/JV_Census_q5")
# saveRDS(JV_Census_q5,
#         file = JV_Census_q5_RDS)
JV_Census_q5 <- readRDS(JV_Census_q5_RDS)