...
LV_rebal_user_only_0619_combined_rowPairs_ct_origin_count <- LV_rebal_user_only_0619_combined_rowPairs_ct %>% 
  group_by(Start.Census.Tract) %>%
  summarise(Ocount=n())
LV_rebal_user_only_0619_combined_rowPairs_ct_count <- merge(LV_Census_geoinfo %>% st_set_geometry(NULL), LV_rebal_user_only_0619_combined_rowPairs_ct_origin_count, by.x='GEOID', by.y='Start.Census.Tract')

LV_rebal_user_only_0619_combined_rowPairs_ct_end_count <- LV_rebal_user_only_0619_combined_rowPairs_ct %>% 
  group_by(End.Census.Tract) %>%
  summarise(Ecount=n())
LV_rebal_user_only_0619_combined_rowPairs_ct_count <- merge(LV_rebal_user_only_0619_combined_rowPairs_ct_count %>% dplyr::select(GEOID, Ocount), LV_rebal_user_only_0619_combined_rowPairs_ct_end_count, by.x='GEOID', by.y='End.Census.Tract')

LV_rebal_user_only_0619_combined_rowPairs_ct_count$diff <- LV_rebal_user_only_0619_combined_rowPairs_ct_count$Ecount - LV_rebal_user_only_0619_combined_rowPairs_ct_count$Ocount

ggplot() +
  geom_sf(data = LV_Census_geoinfo, fill='grey') +
  geom_sf(data=LV_rebal_user_only_0619_combined_rowPairs_ct_count %>%
...
# extract data
LV_rebal_user_only_0619_most_flow_out <- LV_rebal_user_only_0619_combined_rowPairs_ct %>%
...
ggplot()+
  ...
  geom_segment(data = LV_rebal_user_only_0619_most_flow_out, aes(x=LV_Census_geoinfo$centroid_X[LV_Census_geoinfo$GEOID=='21111007100'], y=LV_Census_geoinfo$centroid_Y[LV_Census_geoinfo$GEOID=='21111007100'],xend=lon_d, yend=lat_d, alpha=cnt), col="white", size=.7) +
  scale_alpha_continuous(range = c(0.03, 0.3))+
  labs(title = 'Census tract with highest out flow', xlab('longitude'), ylab('latitude'),
       subtitle = 'GEOID: 21111007100') +
  mapTheme()

LV_rebal_user_only_0619_most_flow_in1 <- LV_rebal_user_only_0619_combined_rowPairs_ct %>%
...
ggplot()+
  ...
  geom_segment(data = LV_rebal_user_only_0619_most_flow_in1, aes(x=LV_Census_geoinfo$centroid_X[LV_Census_geoinfo$GEOID=='21111005900'], y=LV_Census_geoinfo$centroid_Y[LV_Census_geoinfo$GEOID=='21111005900'],xend=lon_d, yend=lat_d, alpha=cnt), col="white", size=.7) +
  scale_alpha_continuous(range = c(0.03, 0.3))+
