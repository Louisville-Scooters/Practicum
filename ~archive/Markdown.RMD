...
The Rebalance Data is a dataset that lists every "status change" for every scooter operated in Louisville between November 2018 and December 2019.

Each row is a trip end. _E.g._ one row is a pickup at a certain location, and the next row is the dropoff, though the data is not always so clean, and events that should be paired do not have a corresponding entry.

### User Events

* Limit data to "user" events, _i.e._ riders picking up and dropping off a scooter.
...
User Activity is highest in the mid-day and evening and peaks on Saturdays.
...
Rebalancing activity primarily occurs in the early morning and late night hours.
...
The flow maps below show the census tracts with the highest inflow of scooters from rebalancing.

![Rebalance inflow](rebalance_inflow_combined.png)


### Are scooter providers complying with rebalancing policy?
...
With the goal of promoting equity of transportation access, the City of Louisville requires scooter providers operating more than a certain number of vehicles to distribute a percentage of those vehicles to particular zones. Per the city's dockless vehicle policy: 

> Distribution zones are intended to ensure that no singular zone is intentionally over-served or under-served.

For operators with more than 150 vehicles, 20% of vehicles must be in zones 1 and 9. For operators with more than 350 vehicles, an additional 10% of scooters must be in zone 8.
...
Currently, Bird and Lime are the only operators permitted to operate more than 150 scooters in Louisville, with each having a limit of 450.  However, this doesn't necessarily mean they are deploying fleets up to the full limit.

While the Rebalance Data tell us about vehicle flows -- what vehicles have entered and exited the system and where are they moving -- they don't at first glance tell us about the status of the entire fleet. That a vehicle hasn't moved in a while and doesn't appear in the Rebalance Data on a particular day doesn't mean it's not somewhere in the city ready to be ridden.

However, by tracking the most recent status of each vehicle at any given time, we can get a snapshot at any point in time with the locations of every available scooter in Louisville.

Process:
    1. Assign an `active`, `reserved`, and `inactive` status for every event type.
    2. Find a time interval to audit. For this vignette, we look at noon at mid-month for every month from the beginning of scooter service in Louisville (November 2018) to December 2019.
    3. For each vehicle in the dataset (determined by unique `vehicleId`), find its most recent status at each audit time.
        * If the scooter's status is `active`, return its location.
        * Set a `buffer` time. If the scooter hasn't had a status change in the last 10 days, consider it `inactive` and removed from the fleet.
    4. For every active scooter, determine which distribution zone it's located in.
    5. Summarize the vehicles by distribution zone and operator to see who is complying with the rebalancing requirements.
...
Generally, Bird and Lime do not comply with the requirements. The percentage of available scooters in Zones 1, 8, and 9 are much lower than the threshold.

**Internal Note:** Need to account for how vehicle fleet fluctuates according to our metric. If "active" fleet is less than 350 at a given time, Zone 8 requirement doesn't apply.
...
If we map the locations of active scooters on November 15, 2019, we'll see that few of them are located in the required distribution zones.
...
Another way we've conceptualized equity of access to scooters is through an Opportunity Index. We define the OI as:

`Opportunity Index = (Rebalance In) + (User Trips In)`

OI is a proxy for the supply of scooters available in a census tract over a certain period of time. It does not take scooter demand in an area into consideration.

Below is OI for all census tracts aggregated over a week in June 2019. Downtown Louisville has the highest supply of scooters irrespective of demand.
...
Net Scooter Flow is the Opportunity Index subtracted by User Trips Out. 

`Net Scooter Flow = (Opportunity Index) - (User Trips Out)`

The below shows that the census tracts with the highest OI also have the highest Net Scooter Flow.
...
ggplot() +
  ...
  scale_fill_viridis() +
  mapTheme() +
  labs(title = "Net Scooter Flow by Census Tract in Louisville")
```
...
## Full Louisville Rebalancing Policy

For reference, the full policy is linked and summarized below:

[Link to policy](https://data.louisvilleky.gov/sites/default/files/Dockless_Policy_7_12_2019.pdf)

* Vehicle Limits
    + General
        - A total of 8 operators will be permitted to operate within Metro designated operational zones.
        - Fleet increases subject to general compliance with all policy provisions
        - Calculations for assessing fleet size increases shall follow this formula:
          `rides per vehicle per day = total rides / total permitted vehicles`
    + For Full-Operating Permit Holders
        - Operators may apply for possible incremental increases of 200 vehicles per month provided that:
            1. Ridership exceeds 3 rides per vehicle per day on vehicles owned by that operator.
        ...
        - Fleet Reductions:
            1. If ridership falls below an average of 2 rides per vehicle per day in the preceding month, the operator must remove vehicles from service. The specific number of vehicles to be removed will be determined by Metro based upon the available data and contributing factors (seasonal shifts, etc.).
        - Cap Extensions
            1. Vehicle caps may be raised at the discretion of Metro. Considerations factoring into raising the caps are as follows:
                * Ridership exceeds 5 rides per vehicle per day on average over the preceding month.
                * Issues identified by Metro are being adequately addressed to Metro’s satisfaction.
    + Current Limits
        - Bird - 450 max vehicles/day - launched August 2018 (_According to the data, they launch in August 2019_)
